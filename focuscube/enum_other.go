...
// vidFTDI is the FocusCube's FTDI bridge USB vendor ID, as the enumerator reports
// it (a hex string; matched case-insensitively).
const vidFTDI = "0403"
...
func enumeratePorts() ([]DeviceInfo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("focuscube: enumerate ports: %w", err)
	}
	var out []DeviceInfo
	for _, p := range ports {
